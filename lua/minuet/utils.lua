...
function M.expand_template(template_str, values, eval)
    ...
    while true do
        ...
        local value = eval(values[template_str:sub(start_pos + 3, end_pos - 3)])
...
function M.make_system_prompt(template, n_completion)
    local system_prompt = M.get_or_eval_value(template.template)
    local n_completion_template = M.get_or_eval_value(template.n_completion_template)
    ...
    return M.expand_template(system_prompt, template, M.get_or_eval_value)
end
...
function M.make_chat_llm_shot(context, template)
    ...
    local function eval(value)
        if type(value) == 'function' then
            return value(context.lines_before, context.lines_after, context.opts)
        end
        return value
    end
